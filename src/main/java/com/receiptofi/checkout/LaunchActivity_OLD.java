...
import com.receiptofi.checkout.http.API.key;
...
public class LaunchActivity_OLD extends Activity implements OnClickListener, ConnectionCallbacks, OnConnectionFailedListener {
    ...
    private void authenticateUser(boolean isSocialLogin, Bundle data) {
        if (isSocialLogin) {
            ...
            try {
                postData.put(key.PID, data.getString(key.PID));
                postData.put(key.ACCESS_TOKEN, data.getString(key.ACCESS_TOKEN));
                    ...
                    if (headers != null) {
                        for (Header header : headers) {
                            ...
                            if (key != null && (key.trim().equals(API.key.XR_MAIL) || key.trim().equals(API.key.XR_AUTH))) {
                                ...
                                KeyValue.insertKeyValue(LaunchActivity_OLD.this, key, value);
                            }
                        }
                    }
                    afterSuccessfullLogin();
    ...
    public class AccessTokenGooglePlus extends AsyncTask<Void, Void, Void> {
        ...
        String scopes = "audience:server:client_id:" + key.SERVER_CLIENT_ID;
        ...
        protected Void doInBackground(Void... params) {
            ...
            try {
                ...
                // application.
                token = GoogleAuthUtil.getToken(LaunchActivity_OLD.this, Plus.AccountApi.getAccountName(mGoogleApiClient), scopes);
            } catch (Exception e) {
