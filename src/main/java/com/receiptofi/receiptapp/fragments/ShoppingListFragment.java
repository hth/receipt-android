...
import com.receiptofi.receiptapp.adapters.ShoppingListAdapterF;
...
public class ShoppingListFragment extends Fragment {
    ...
    private ShoppingListAdapterF mAdapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (upId > 0) {
            ImageView up = (ImageView) rootView.findViewById(upId);
            up.setImageDrawable(new IconDrawable(getActivity(), FontAwesomeIcons.fa_chevron_left)
    ...
    private void setupView() {
        fbAddTag = (FloatingActionButton) rootView.findViewById(R.id.buttonFloat);
        ...
        Bundle receiveBundle = getArguments().getBundle(BUNDLE_KEY_BIZ_NAME);
    ...
    private void setupView(List<ShoppingItemModel> shoppingItemModels) {
        if (!shoppingItemModels.isEmpty()) {
            mListView = (SwipeMenuListView) rootView.findViewById(R.id.shopping_list_listView);
            mAdapter = new ShoppingListAdapterF(getActivity(), this, shoppingItemModels);
            mListView.setAdapter(mAdapter);
            addAnimation();

            TextView textView = (TextView) rootView.findViewById(R.id.shopping_place);
